...
import '../account/account_actions.dart' as account_actions;
...
class LNUrlBloc with AsyncActionsHandler {
    ...
    if (_lnUrlStreamController == null) {
      ...
      Observable.merge([
        ...
        if (response.runtimeType == LNUrlResponse) {
          ...
          } else if (response.hasPay()) {
            _lnUrlStreamController.add(PayFetchResponse(response.pay));
          } else {
  ...
  Future _fetch(Fetch action) async {
    ...
    if (res.hasPay()) {
      action.resolve(PayFetchResponse(res.pay));
      return;
    }
